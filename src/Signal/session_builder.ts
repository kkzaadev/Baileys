...
export class SessionBuilder {
	...
	async initOutgoing(device: {
		...
		return mutex.acquire().then(async release => {
			try {
				...
				Curve.verify(device.identityKey, device.signedPreKey.publicKey, device.signedPreKey.signature)
	...
	async initIncoming(record: SessionRecord, message: proto.PreKeySignalMessage): Promise<number | undefined> {
		...
		if (!(await this.storage.isTrustedIdentity(fqAddr, message.identityKey))) {
			throw new errors.UntrustedIdentityKeyError(this.addr, bytesToBase64(message.identityKey))
		}
	...
	async initSession(
		...
		const a1 = Curve.sign(theirSignedPubKey!, ourIdentityKey.privKey)
		const a2 = Curve.sign(theirIdentityPubKey, ourSignedKey!.privateKey)
		const a3 = Curve.sign(theirSignedPubKey!, ourSignedKey!.privateKey)
		if (isInitiator) {
		...
		if (ourEphemeralKey && theirEphemeralPubKey) {
			const a4 = Curve.sign(theirEphemeralPubKey, ourEphemeralKey.privateKey)
			sharedSecret.set(new Uint8Array(a4), 32 * 4)
		}

		const masterKey = await deriveSecrets(Buffer.from(sharedSecret), Buffer.alloc(32), Buffer.from('WhisperText'))
	...
	async calculateSendingRatchet(session: SessionEntry, remoteKey: Uint8Array) {
		...
		const masterKey = await deriveSecrets(sharedSecret, ratchet.rootKey, Buffer.from('WhisperRatchet'))
