...
export class SessionCipher {
	...
	public async encrypt(data: Uint8Array): Promise<{ type: number; body: Uint8Array; registrationId: number }> {
		...
		return mutex.acquire().then(async release => {
			try {
				...
				if (!(await this.storage.isTrustedIdentity(this.addr.id, remoteIdentityKey))) {
					throw new errors.UntrustedIdentityKeyError(this.addr.id, bytesToBase64(remoteIdentityKey))
				}

				const chain = session.getChain(session.currentRatchet.ephemeralKeyPair.publicKey)
				...
				msg.ciphertext = await encryptCBC(keys[0], data, keys[2].slice(0, 16))
	...
	public async decryptPreKeyWhisperMessage(data: Uint8Array): Promise<Uint8Array> {
	...
	private async doDecryptWhisperMessage(messageBuffer: Uint8Array, session: SessionEntry): Promise<Uint8Array> {
		if (!session) {
		...
		const keys = await deriveSecrets(messageKey, Buffer.alloc(32), Buffer.from('WhisperMessageKeys'))
		const ourIdentityKey = await this.storage.getOurIdentity()
		...
		const plaintext = await decryptCBC(keys[0], message.ciphertext, keys[2].slice(0, 16))
	...
	private async fillMessageKeys(chain: any, counter: number): Promise<void> {
		...
		chain.messageKeys[chain.chainKey.counter + 1] = await calculateMAC(key, Buffer.from([1]))
		chain.chainKey.key = await calculateMAC(key, Buffer.from([2]))
		chain.chainKey.counter += 1
		return this.fillMessageKeys(chain, counter)
	}

	private async maybeStepRatchet(session: SessionEntry, remoteKey: any, previousCounter: any): Promise<void> {
	...
	private async calculateRatchet(session: SessionEntry, remoteKey: any, sending: boolean): Promise<void> {
		const ratchet = session.currentRatchet
		const sharedSecret = Curve.sign(remoteKey, ratchet.ephemeralKeyPair.privateKey)
		const masterKey = await deriveSecrets(sharedSecret, ratchet.rootKey, Buffer.from('WhisperRatchet'), /*chunks*/ 2)
