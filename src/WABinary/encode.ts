...
class BinaryEncoder {
	...
	private pushString(str: string) {
		this.ensureCapacity(str.length * 3)
		const { written } = TEXT_ENCODER.encodeInto(str, this.buffer.subarray(this.offset))
		this.offset += written
	}
